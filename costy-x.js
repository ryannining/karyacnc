...
function getvalue(el) {
    ...
    else if (el == "leadin") return $(el).value.split(",");
    else if (el == "pup" || el == "pdn") return $(el).value.replaceAll(";","\n");
...
function gcodepush(lenmm, X1, Y1, lenmm1, line, closed) {
    ...
    if (closed) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            gcodes.push([lenmm, "", x1, y1, lines[i], srl, ar, closed, shapenum,cuttab_manual[i]]);
        }
...
//draw_line      (num, lcol,      lines,          srl, dash, len, closed, snum,         flip,shift,warn) {
    ...
    pup1 = getvalue("pup") + '\n';
    startgcode="M3";
    finishgcode =($("spindleoff").checked)?"M3 S0\n":"";
...
function sortedgcode() {
    ...
    for (var j = 0; j < sgcodes.length; j++) {
        ...
        if (disable_cut.indexOf(sgcodes[j][1] + "") < 0) {
            ...
            var acp="";
            if ($("spindleoff").checked)acp="M3 S0\n";
            acp += ps;
            var isacp = 0;
            ...
            if (_re && spiraldown && !ismark) {
                //s += lines2gcode(sgcodes[j][1], sgcodes[j][0], cncz2, cncz2, vcuttab, sgcodes[j][0][5], 1, 0, snum, f2, 0,shift,rcutpos);
            }
            //s+=pup1+"\n\n";
        }
    }
